...
const SystemPOList = ({ refreshList, onEdit, onDelete }) => {
  ...
  const fetchSystemPOs = async () => {
    try {
      ...
      const mappedPOs = data.map((po) => ({
        ...
        // INCLUDE items + tna_created
        items: Array.isArray(po.items) && po.items.length > 0
          ? po.items.map(item => ({
              ...item,
              cost_sheet_code: item.cost_sheet_code || "",
              sku_code: item.sku_code || "",
              tna_created: item.tna_created || false, // <-- IMPORTANT
            }))
          : [],
      }));
  ...
  try {
    await axios.delete(`${API_BASE}/systempos/${system_po_id}`);
    alert("System PO deleted successfully!");
    if (onDelete) onDelete();
    fetchSystemPOs();
  } catch (err) {
    console.error("Error deleting System PO:", err);

    // Show backend error if TNA exists
    if (err.response && err.response.data && err.response.data.message) {
      alert(err.response.data.message); // e.g., "Cannot delete System PO because TNA records exist"
    } else {
      alert("Failed to delete System PO");
    }
  }
};
  ...
  const handleCreateTNA = async (system_po_id) => {
    try {
      const response = await axios.post(`${API_BASE}/tna/systempos/${system_po_id}/generate-tna`);
      alert(response.data.message || "TNA records created successfully!");
      fetchSystemPOs(); // <-- REFRESH LIST to update tna_created
    } catch (err) {
      console.error("Error creating TNA:", err);
      alert(err?.response?.data?.error || "Failed to create TNA");
    }
      ...
      <div className="max-w-full mx-auto overflow-x-auto bg-white rounded-lg shadow-md">
        ...
        {systemPOs.length === 0 ? (
          ...
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {systemPOs.map((po) => {
                // CHECK IF ANY ITEM HAS tna_created = true
                const isTnaCreated = po.items?.some(item => item.tna_created);
                  ...
                  <tr key={po.system_po_id}>
                    ...
                    <td className="px-4 py-2 text-center">
                      <div className="flex justify-center space-x-2">
                        ...
                        {/* CREATE TNA BUTTON WITH DISABLE LOGIC */}
                        <button
                          onClick={() => handleCreateTNA(po.system_po_id)}
                          disabled={isTnaCreated}
                          className={`px-3 py-1 text-sm rounded 
                            ${isTnaCreated
                              ? "bg-gray-400 cursor-not-allowed text-white"
                              : "bg-blue-500 text-white hover:bg-blue-600"}`}
                        >
                          {isTnaCreated ? "TNA Created" : "Create TNA"}
                        </button>
