  ...
  useEffect(() => {
    if (editingCostSheet) {
      setFormData({
        ...
        margin:editingCostSheet.margin || "",
...
useEffect(() => {
  const fetchExchangeRate = async () => {
    if (!formData.currency_master) {
      // No currency selected yet, do nothing
      return;
    }
    
    if (formData.currency_master === "INR") {
      setFormData((prev) => ({
        ...prev,
        exchange_rate: "1",
      }));
    ...
    try {
      const res = await axios.get(
        `${API_BASE}/forex/${formData.currency_master}`
      );

      setFormData((prev) => ({
        ...prev,
        exchange_rate: res.data.rate.toString(),
      }));
    } catch (err) {
...
//-----------------------------------------------------------
  ...
  useEffect(() => {
    ...
    const exchange=parseFloat(formData.exchange_rate) || 0;

    if (cost > 0 && final > 0) {
      const totalGpValue = final - cost;
      const gpValue = (((final * exchange) - (cost * exchange)) / (cost * exchange)) * 100;
      const marginvalue=(((final*exchange)-(cost*exchange))/(final*exchange))*100;
  ...
  }, [formData.cost_price, formData.final_price,formData.exchange_rate]);
...
const removeStoresOrGrinderiesRow = (index) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="w-full max-w-6xl">
      ...
      <div className="flex items-center mb-4">
        <label className="w-40 text-sm font-medium text-gray-700">Cost Price(INR):</label>
        <input
          type="number"
          step="0.01"
          name="cost_price"
      ...
      <div className="flex items-center mb-4">
        <label className="w-40 text-sm font-medium text-gray-700">Final Price(INR):</label>
        <input
          type="number"
          step="0.01"
          name="final_price"
      ...
      <div className="flex items-center mb-4">
        <label className="w-40 text-sm font-medium text-gray-700">Markup(%):</label>
        <input
          type="number"
          step="0.01"
          name="gp"
      ...
      <div className="flex items-center mb-4">
        <label className="w-40 text-sm font-medium text-gray-700">Margin(%):</label>
        <input
          type="number"
          step="0.01"
          name="margin"
